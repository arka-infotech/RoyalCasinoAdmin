...
import { useDistributors, useDeleteUser, useBlockUser, useUnblockUser } from "@/hooks/useUsers";
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-base font-semibold text-gray-900">Delete Distributor</h2>
...
function DisableModal({ user, onClose, onConfirm, isPending }: { user: User; onClose: () => void; onConfirm: () => void; isPending: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-base font-semibold text-gray-900">{isBlocked ? "Enable" : "Disable"} Distributor</h2>
...
export default function DistributorPage() {
  ...
  const { data, isLoading, error } = useDistributors(parentId);
  const deleteMutation = useDeleteUser();
  const blockMutation = useBlockUser();
  const unblockMutation = useUnblockUser();

  useEffect(() => { if (error) toast.error("Failed to load distributors"); }, [error]);
  ...
  function handleAction(action: ActionKey, row: User) {
    ...
    else if (action === "view") router.push(`/management/credit-adjust/${row.id}?entity=distributor&username=${encodeURIComponent(row.username)}&credits=${row.chips}&parentBalance=${row.parent_chips ?? 0}`);
    else if (action === "changePassword") router.push(`/management/credit-transfer/${row.id}?entity=distributor&username=${encodeURIComponent(row.username)}&credits=${row.chips}&parentBalance=${row.parent_chips ?? 0}`);
    else if (action === "edit") router.push(`/management/distributor/edit/${row.id}`);
  }
    ...
    <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:p-6">
      ...
      {parentId && parentName && (
        <HierarchyBreadcrumb
          ...
          returnTo={returnTo ?? "/management/super-distributor"}
        ...
        <table className="min-w-full border border-gray-200 text-sm">
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={7} className="border border-gray-200 px-3 py-8 text-center text-gray-500">
                {searchQuery ? "No results found." : parentName ? `No distributors under ${parentName}.` : "No distributors yet."}
              </td></tr>
            ) : (
