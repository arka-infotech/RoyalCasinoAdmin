  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-base font-semibold text-gray-900">Delete Retailer</h2>
...
function DisableModal({ user, onClose, onConfirm, isPending }: { user: User; onClose: () => void; onConfirm: () => void; isPending: boolean }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-sm rounded-xl bg-white p-6 shadow-xl">
        <h2 className="text-base font-semibold text-gray-900">{isBlocked ? "Enable" : "Disable"} Retailer</h2>
...
export default function RetailerPage() {
  ...
  useEffect(() => { if (error) toast.error("Failed to load retailers"); }, [error]);
  ...
  function handleAction(action: ActionKey, row: User) {
    ...
    else if (action === "view") router.push(`/management/credit-adjust/${row.id}?entity=retailer&username=${encodeURIComponent(row.username)}&credits=${row.chips}&parentBalance=${row.parent_chips ?? 0}`);
    else if (action === "changePassword") router.push(`/management/credit-transfer/${row.id}?entity=retailer&username=${encodeURIComponent(row.username)}&credits=${row.chips}&parentBalance=${row.parent_chips ?? 0}`);
    else if (action === "edit") router.push(`/management/retailer/edit/${row.id}`);
  }

  return (
    <section className="rounded-xl border border-gray-200 bg-white p-4 shadow-sm md:p-6">
      <div className="mb-5 flex items-center justify-between">
        <h1 className="text-lg font-semibold text-gray-900 md:text-xl">Retailer</h1>
        <Link href="/management/retailer/add" className="rounded bg-indigo-500 px-4 py-2 text-xs font-semibold text-white transition hover:bg-indigo-600 md:text-sm">Add Retailer</Link>
      </div>
        ...
        <table className="min-w-full border border-gray-200 text-sm">
          <thead>
            <tr className="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-600">
              {["No", "Username", "Refer Name", "Unique ID", "Points", "Date & Time"].map((h) => (
          ...
          <tbody>
            {isLoading ? (
              ...
              <tr><td colSpan={7} className="border border-gray-200 px-3 py-8 text-center text-gray-500">{searchQuery ? "No results found." : "No retailers yet."}</td></tr>
            ) : (
