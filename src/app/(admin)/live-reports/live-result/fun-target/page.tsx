...
import {
  calcFunTargetExpectedPayment,
  fetchLiveResultStatus,
  formatAdminMoney,
  formatFunTargetHistoryEntry,
  FUN_TARGET_GAME_TYPE,
  getLuckyTimerLabel,
...
const GAME_TYPE = FUN_TARGET_GAME_TYPE;

/** Display order matches the reference UI: 1–5 / 6–9, 0 */
const BOARD_NUMBERS = ["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"] as const;
...
export default function FunTargetLiveResultPage() {
  ...
  const selectDigit = useCallback((digit: string) => {
    if (!isFunTargetDigit(digit)) return;
    setSelected(digit);
    setResultInput(digit);
  }, []);
  ...
  const payload = useMemo(() => {
    ...
    const totalExpectedPayment = previewCard
      ? calcFunTargetExpectedPayment(totals, previewCard, safeReward)
      : null;
    ...
    const recentResults = (live?.last_win_cards ?? [])
      .slice(0, 12)
      .map(formatFunTargetHistoryEntry);
  ...
  async function handleSave() {
    setSaveMsg(null);
    const winCard = selected || (isFunTargetDigit(resultInput) ? resultInput : "");
    if (!winCard) {
      setSaveMsg("Select or enter a digit (0–9)");
      return;
    }
    setSaveBusy(true);
    try {
  ...
  return (
    <section className="w-full min-w-0 overflow-x-hidden rounded-xl border bg-white p-2 shadow-sm sm:p-3 md:p-4">
      <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-4">
        <h1 className="text-[14px] font-medium text-gray-900">Fun Target</h1>
        <button
          type="button"
          disabled={resetBusy}
      ...
      <div className="mt-3 grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,1fr)_minmax(220px,320px)] xl:items-start xl:gap-6">
        <div className="order-2 min-w-0 xl:order-1">
          <div className="mx-auto w-full max-w-[760px] overflow-hidden rounded-[3px] border border-gray-200 bg-[#F3F3F3] p-1.5 sm:p-3 md:p-4">
            <div className="grid grid-cols-5 gap-1 min-[400px]:gap-1.5 sm:gap-2.5 md:gap-4">
              {BOARD_NUMBERS.map((digit) => {
                const stake = payload.totals[digit] ?? 0;
                const isSelected = selected === digit;
                return (
                  <div key={digit} className="min-w-0">
                    <label
                      className={cn(
                        "relative flex aspect-square w-full min-h-0 cursor-pointer items-center justify-center rounded-[2px] border bg-white shadow-[0_1px_0_rgba(0,0,0,0.04)]",
                    ...
                    >
                      {canSelect ? (
                        <input
                          type="radio"
                          name="fun-target-selection"
                          checked={isSelected}
                          onChange={() => selectDigit(digit)}
                      ...
                      <span className="text-[clamp(1.15rem,4.6vw,2.75rem)] font-semibold leading-none text-gray-900 tabular-nums">
                        {digit}
                      </span>
                    </label>
          ...
          <div className="mt-3 flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            <input
              ...
              onChange={(e) => {
                ...
                setResultInput(next);
                setSelected(isFunTargetDigit(next) ? next : "");
              }}
              placeholder="0–9"
