...
export default function SpinToWinLiveResultPage() {
  ...
  const selectDigit = useCallback((digit: string) => {
    if (!isSpinToWinDigit(digit)) return;
    setSelected(digit);
  ...
  const payload = useMemo(() => {
    ...
    const totalExpectedPayment = previewCard
      ? calcSpinToWinExpectedPayment(totals, previewCard, safeReward)
      : null;
    ...
    const recentResults = (live?.last_win_cards ?? [])
      .slice(0, 12)
      .map(formatSpinToWinHistoryEntry);
  ...
  async function handleSave() {
    setSaveMsg(null);
    const winCard = selected || (isSpinToWinDigit(resultInput) ? resultInput : "");
    if (!winCard) {
  ...
  return (
    <section className="w-full min-w-0 overflow-x-hidden rounded-xl border bg-white p-2 shadow-sm sm:p-3 md:p-4">
      <div className="flex flex-wrap items-center justify-between gap-2 sm:gap-4">
        <h1 className="text-[14px] font-medium text-gray-900">Spin To Win</h1>
        <button
          type="button"
          disabled={resetBusy}
          ...
          <div className="mt-3 flex flex-wrap items-center justify-center gap-2 sm:gap-3">
            <input
              ...
              onChange={(e) => {
                ...
                setResultInput(next);
                setSelected(isSpinToWinDigit(next) ? next : "");
              }}
              placeholder="0–9"
